 ...
 */
#define USB_CFG_DMINUS_BIT      7 // changed for USnooBie
 ...
 */
#define SIGNATURE_BYTES             0x1e, 0x95, 0x0f, 0     /* ATMega328p */
...
#define BOOTLOADER_INIT_BIT      4

static inline void  bootLoaderInit(void)
{
    PORTD |= (1 << BOOTLOADER_INIT_BIT);     /* activate pull-up */ 
    //if(!(MCUCSR & (1 << EXTRF)))    /* If this was not an external reset, ignore */
    //    leaveBootloader();
    MCUCSR = 0;                     /* clear all reset flags for next time */
		
		Clock_CLKPS(CLOCK_CLKPS_DIV1);
		Clock_OSCCAL(213);
		DDRB |= 1;
		PORTB |= 1;
...
static inline void  bootLoaderExit(void)
{
    PORTD = 0;                      /* undo bootLoaderInit() changes */
		...
		PORTB = 0;
		DDRB = 0;		
		Clock_CLKPS(CLOCK_CLKPS_DIV8);
		Clock_OSCCAL(0x8A);
}

#define bootLoaderCondition()   ((PIND & (1 << BOOTLOADER_INIT_BIT)) == 0)
